#-----------------------------------------------------------------------------------------------------------------------
# Program: HEX Grid Tessellator — Build
# Version: 3.1.0
# Author:  Rohin Gosling
#
# Description:
#
#   Builds the standalone Windows executable (dist/hextessellator.exe) via PyInstaller, then smoke-tests it. The build
#   stamps the changelog version and today's build date into src/main.py, compiles using hextessellator.spec, and runs
#   the frozen exe to confirm it produces a non-empty PNG.
#
# Usage:
#
...
PROJECT_DIRECTORY = os.path.dirname ( os.path.dirname ( os.path.abspath ( __file__ ) ) )   # Project root (parent of scripts/).
MAIN_PY           = os.path.join ( PROJECT_DIRECTORY, "src", "main.py" )                    # Entry module that is stamped.
CHANGELOG         = os.path.join ( PROJECT_DIRECTORY, "docs", "CHANGELOG.md" )              # Source of the version number.
SPEC_FILE         = os.path.join ( PROJECT_DIRECTORY, "hextessellator.spec" )               # PyInstaller spec.
EXE_PATH          = os.path.join ( PROJECT_DIRECTORY, "dist", "hextessellator.exe" )        # Expected build output.
...
# Description:
#
#   Return the highest versioned heading from docs/CHANGELOG.md, scanning for "## [X.Y.Z]" and skipping "[Unreleased]".
#
# Arguments:
#
#   None.
...
def step_stamp_version () -> tuple [ bool, str ]:
    ...
    pattern            = r'(_changelog_version\s*\(\s*")[^"]*("\s*\))'
...
def step_stamp_build_date () -> tuple [ bool, str ]:
    ...
    pattern            = r'(BUILD_DATE\s*:\s*str\s*=\s*")[^"]*(")'
...
# Description:
#
#   Build the one-file executable from hextessellator.spec via PyInstaller.
#
# Arguments:
#
#   None.
...
def step_build_exe () -> tuple [ bool, str ]:
    ...
    result = subprocess.run (
        ...
        capture_output = True, text = True, cwd = PROJECT_DIRECTORY
    )
...
# Description:
#
#   Run the frozen exe in a temporary directory and confirm it produces a non-empty PNG.
#
# Arguments:
#
#   None.
...
def step_smoke_test_exe () -> tuple [ bool, str ]:
    ...
    with tempfile.TemporaryDirectory () as temp_dir:
        ...
        output_png = os.path.join ( temp_dir, "smoke_test.png" )
        env        = { **os.environ, "PYTHONIOENCODING": "utf-8" }

        result = subprocess.run (
            [ EXE_PATH, "--file", output_png, "--antialias", "off", "--layers", "1", "--debug" ],
        ...
        if not os.path.isfile ( output_png ):
            return False, ".exe ran but did not produce the expected PNG"

        size = os.path.getsize ( output_png )

        if size == 0:
            return False, ".exe produced an empty PNG"

        # Return data to caller.

        return True, f".exe produced a {size}-byte PNG"
...
def main () -> int:
    ...
    print_banner ( "HEX Grid Tessellator - Build" )
    print ()

    steps = [
        ( "Stamping version from docs/CHANGELOG.md", step_stamp_version ),
        ( "Stamping build date", step_stamp_build_date ),
        ( "Building .exe via hextessellator.spec", step_build_exe ),
    ]

    all_passed = True

    # Run the stamp and build steps in order.
    ...
    # Smoke-test only if every prior step succeeded.
